(() => {
  ...
  const boostList = document.getElementById("boostStack");
  ...
  if (!calcBtn || !scoreEl || !suggestEl || !haveEl || !missingEl || !boostList || !simulateBtn || !simulateResult || !selectedJobDetail) {
    return;
  }

  let lastResult = null;
  let profileStatePromise = null;
  let selectedBoostStacks = [];
  ...
  const renderBoostOptions = (stacks) => {
    ...
    if (!stacks.length) {
      const empty = document.createElement("span");
      empty.className = "muted";
      empty.textContent = "부족 스택이 없습니다.";
      boostList.appendChild(empty);
      return;
    }
    ...
    stacks.forEach((stack) => {
      ...
      button.type = "button";
      button.className = "boost-stack-chip";
      button.textContent = stack;
      button.addEventListener("click", () => {
        const key = normalize(stack);
        if (selectedBoostStacks.some((item) => normalize(item) === key)) {
          selectedBoostStacks = selectedBoostStacks.filter((item) => normalize(item) !== key);
          button.classList.remove("active");
        } else {
          selectedBoostStacks.push(stack);
  ...
  const getMessageByScore = (score, missingCount) => {
    ...
    if (score >= 80) return "지원 가능한 수준입니다. 부족 스택만 보완해 보세요.";
    if (score >= 60) return "주요 스택은 어느 정도 맞습니다. 부족 스택 보완이 필요합니다.";
  ...
  const calculate = () => {
    ...
    if (!jobStacks.length) {
      scoreEl.textContent = "--%";
      suggestEl.textContent = "세부 설정에서 공고를 선택한 뒤 계산해 주세요.";
      renderTagList(haveEl, [], "보유 스택이 없습니다.");
      renderTagList(missingEl, [], "부족 스택이 없습니다.");
      renderBoostOptions([]);
      simulateResult.textContent = "부족 스택을 선택하면 예상 점수를 계산해 드립니다.";
      lastResult = null;
      return;
    }
  ...
  simulateBtn.addEventListener("click", async () => {
    ...
    if (!lastResult) {
      simulateResult.textContent = "먼저 지수 계산을 실행해 주세요.";
      return;
    }

    if (!selectedBoostStacks.length) {
      simulateResult.textContent = "부족 스택을 하나 이상 선택해 주세요.";
      return;
    }

    const improvedCount = Math.min(lastResult.haveCount + selectedBoostStacks.length, lastResult.total);
    const nextScore = Math.round((improvedCount / lastResult.total) * 100);
    simulateResult.textContent = `${selectedBoostStacks.join(", ")} 스택을 보완하면 예상 매칭 점수는 ${nextScore}%입니다.`;
  });
})();
